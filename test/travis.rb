...
stdout, stderr, status = Open3.capture3 "sass --load-path ./bower_components/ --style compressed --scss main.scss test/output/main.css"
...
stdout, stderr, status = Open3.capture3 "sass --load-path ./bower_components/ --style compressed --scss test/not-silent.scss test/output/not-silent.css"
raise stderr unless status.success?
raise result unless $?.to_i == 0
